...
export function ToolDetailPage({ section = 'overview' }: { section?: 'overview' | 'runs' }) {
  ...
  function setSelectedRun(value: string) {
    setSearch((previous) => {
  ...
  const [actionError, setActionError] = useState<string | null>(() => {
    ...
    if (result?.outcome === 'saved_launch_failed') return result.state?.message || 'The tool was saved, but setup or launch failed. Review the Runs tab and check Environment to retry setup.'
  ...
  const moreMenuItems: OverflowMenuItem[] = [
    ...(current.projectPath ? [{ id: 'share', label: 'Share tool…', disabled: busy, onSelect: share }] : []),
    {
      id: 'restart',
      label: 'Restart',
      disabled: busy,
  ...
  return (
    <>
      <ToolPageHeader id={toolId} name={current.name} description={current.description} runId={selectedRun}>
        <Link className="btn btn-quiet" to={`/tools/${toolId}/edit`}>Edit tool</Link>
      </ToolPageHeader>
      {actionError ? (
      ...
      {section === 'overview' && <section className="panel run-summary">
        ...
        <div className="panel-body stack">
          ...
          <Link className="btn btn-quiet btn-sm" to={`/tools/${toolId}/runs`}>View runs and output</Link>
      ...
      {section === 'runs' && (
        <section className="panel">
          <div className="panel-header">
            <h2 className="panel-title" id="run-output" tabIndex={-1}>Run output</h2>
      ...
      {section === 'runs' && <section className="panel">
        <div className="panel-header">
        ...
        <div className="panel-body">
          {receiptError && <p role="alert" className="form-error">{receiptError} <button className="btn btn-sm" onClick={() => void refreshReceipts()}>Retry history</button></p>}
          <ReceiptHistory
            receipts={receipts}
            onInspect={(receipt) => { setSelectedRun(receipt.id); document.getElementById('run-output')?.focus() }}
            emptyLabel="No launches recorded yet. Launch this tool to start its history."
          />
        </div>
      </section>}
      {section === 'overview' && <ToolEnvironmentPanel key={tool.id} tool={tool} />}
